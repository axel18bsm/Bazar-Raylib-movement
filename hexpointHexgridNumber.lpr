...
var
  HexGrid: array[1..Columns*Rows] of THexCell;
...
// permet de creer un hexagon.
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        HexGrid[hexNumber].Center.x := Round(HexRadius + offsetX);
        HexGrid[hexNumber].Center.y := Round(HexHeight / 2 + offsetY);

        // Définit le numéro de l'hexagone
        HexGrid[hexNumber].Number := hexNumber;

        HexGrid[hexNumber].colonne := i;   //colonne
        HexGrid[hexNumber].ligne := j;    //rows
        HexGrid[hexNumber].PairImpairLigne:=PairOuImpair(hexNumber);
        ...
        HexGrid[hexNumber].Selected := False;
  ...
  begin
    ...
    begin
      for j := 1 to Rows do
      begin
        DrawPoly(Vector2Create(HexGrid[hexNumber].Center.x, HexGrid[hexNumber].Center.y),
          6, HexRadius - 1, 30, HexGrid[hexNumber].Color);

        if HexGrid[hexNumber].Selected then
          outlineColor := ORANGE
        else
          outlineColor := DARKGRAY;

        DrawPolyLinesEx(Vector2Create(HexGrid[hexNumber].Center.x, HexGrid[hexNumber].Center.y),
          6, HexRadius, 30, 2, outlineColor);
       if dessineLesNombres=True then
         begin
        StrPCopy(hexNumberText, IntToStr(HexGrid[hexNumber].Number));
        DrawText(hexNumberText, Round(HexGrid[hexNumber].Center.x - 10),
          Round(HexGrid[hexNumber].Center.y - 10), 20, BLACK);
         End;
         end;
      Inc(hexNumber);
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        for j := 1 to Rows do
        begin
          dx := mouseX - HexGrid[hexNumber].Center.x;
          dy := mouseY - HexGrid[hexNumber].Center.y;
          dist := sqrt(dx * dx + dy * dy);
          if dist <= HexRadius-decalageRayon then  //diminution du rayon entre -1 et -2 pour ne pas cliquer sur 2 hexagones joints
          begin
            HexGrid[hexNumber].Selected := True;
            SelectedHex := HexGrid[hexNumber];
            HexSelected := True;
          end
          else
            HexGrid[hexNumber].Selected := False;
