program hexpsourispointusave4okend;

uses
  Math,SysUtils, BoutonClic,raylib;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        Writeln(F, Format('%d,%d,%d,%d,%d,%d,%s,%d,%d,%s,%s,%s,%s',
          [HexGrid[i].Number,
...
begin
  ...
  try
    ...
    begin
      ...
      Writeln(F, Format('  Centre: (%d, %d)', [HexGrid[i].Center.x, HexGrid[i].Center.y]));
      ...
      Writeln(F, Format('  Voisins: %s', [NeighborStr]));
