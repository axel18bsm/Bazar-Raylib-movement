program hexpointokmodelenepastoucher2;
...
const
  ...
  rows = 6;                    // Nombre d'hexagones en largeur
  columns = 5;                       // Nombre d'hexagones en hauteur
...
var
  HexGrid: array[1..rows, 1..columns] of THexCell;                  // colonne et ligne drive la table
  HexGridNumber: array[1..TotalNbreHex] of THexCell;                // le numero d hexagone drive la table
  i, j,k, hexNumber: integer;
...
procedure PositionHexagone();
 ...
 begin
   ...
   begin
      HexGridNumber[i].Poshexagone:=inconnu;       //initialisation du champ
     ...
     HexGridNumber[TotalNbreHex].Poshexagone:=CoinHG;
