...
procedure PositionHexagone();
 ...
 begin
   ...
   begin
     ...
     if HexGrid[i].colonne=rows then HexGrid[i].Poshexagone:=Bordb;        //bordbas derniere ligne car on le connait à l initialisation
   end;
   ...
   // les coins                                                                //on ecrase les coins
     ...
     HexGrid[TotalNbreHex-columns+1].Poshexagone:=CoinBD;                // coin bas gauche
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        HexGrid[hexNumber].PairImpairLigne:=PairOuImpair(hexNumber);            // la ligne pair ou impaire va nous servir à connaitre les voisins
  ...
  begin
    ...
    for i := 1 to TotalNbreHex do                                                // numro de l hexagone

      begin
        DrawPoly(Vector2Create(HexGrid[i].Center.x, HexGrid[i].Center.y),
          6, HexRadius - 1, 30, HexGrid[i].Color);
